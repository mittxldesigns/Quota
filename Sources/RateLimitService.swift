...
class RateLimitService: ObservableObject {
    ...
    // Usage history — learns your patterns over time
    ...
    // Fix #1: Only publish when the string actually changes — prevents NSImage redraw every 5s
    private func updateTimestamp() {
        guard let d = lastUpdated else {
            if lastUpdatedFormatted != "never" { lastUpdatedFormatted = "never" }
            return
        }
        let s = Date().timeIntervalSince(d)
        let newValue: String
        if s < 5  { newValue = "just now" }
        else if s < 60 { newValue = "\(Int(s))s ago" }
        else { newValue = "\(Int(s / 60))m ago" }
        if newValue != lastUpdatedFormatted { lastUpdatedFormatted = newValue }
    }
                ...
                do {
                    ...
                    if let retryHTTP = retryResp as? HTTPURLResponse, retryHTTP.statusCode == 200,
                       let parsed = parseUsageResponse(retryData) {
                        rateLimitData = parsed
                        errorMessage = nil
                        let wasBackoff = consecutiveFailures > 0
                        consecutiveFailures = 0
                        if wasBackoff { restartTimer() }
                        callsWithCurrentToken = 1
                    } else {
    ...
    private static let githubRepo = "mittxldesigns/Quota"
    ...
    func performUpdate() {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            do {
                ...
                // Safety net: if terminate didn't work within 10s, reset UI
                try? await Task.sleep(for: .seconds(10))
                await self?.updateFailed("Restart timed out — please relaunch manually")
    ...
    private func parseISO8601(_ string: String) -> Date? {
        Self.isoFmtFrac.date(from: string) ?? Self.isoFmtBasic.date(from: string)
    }
}
